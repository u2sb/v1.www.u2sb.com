...
[vuepress-plugin-sbmddia](https://github.com/u2sb/vuepress-plugin-sbmddia) 是一个能在 Vuepress 中快速插入媒体标签的插件，目前支持的标签和平台有（持续开发中）：
...
vuepress-plugin-sbmddia 插件和其他 Vuepress 插件使用方法类似，诸如 如何启用一个插件 之类的共性问题就不再赘述了，只说 vuepress-plugin-sbmddia 相关的。
...
<code-group>
  ...
  ```bash
  pnpm install vuepress-plugin-sbmddia -D
  ```
  </code-block>

  <code-block title="YARN">
  ```bash
  yarn add vuepress-plugin-sbmddia -D
  ```
  </code-block>

  <code-block title="NPM">
  ```bash
  npm install vuepress-plugin-sbmddia -D
  ```
  </code-block>
</code-group>
...
[GitHub](https://github.com/u2sb/vuepress-plugin-sbmddia/issues)
